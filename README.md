...
```
**That's roughly a 50% decrease in line size of the original view setup code**, plus we didn't have to deal with any constraints. On top of that, one of the biggest advantages of this framework, is how easy it is to manipulate the views.
Repositioning the elements is as easy as swapping the elements around, as opposed to manually connecting the respective constraints for the affected views. Also, its declarative nature makes it easy to visualize the composition of the view just by looking at the code, very similar to SwiftUI.
...
### What DeclarativeUIKit is and what it is not
